...
def get_config(num_trn) -> config_dict.ConfigDict:
    ...
    config.train_split = f"train[{config.num_val+10_000}:{config.num_val+config.num_trn}]"
    config.pp_train = f'value_range(-1, 1, 0, 1, True)|keep(["image"])'
    config.val_split = f"train[10000:{config.num_val+10_000}]"
    config.pp_eval = (
        f'value_range(-1, 1, 0, 1, True)|move_key("label_shape", "label")|keep(["image", "label"])'
    )
    ...
    config.model.Z_given_X.dropout_rate = 0.0
    config.model.Z_given_X.input_dropout_rate = 0.0
    ...
    config.model.X_given_Z.dropout_rate = 0.0
    config.model.X_given_Z.input_dropout_rate = 0.0
