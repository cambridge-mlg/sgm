"""Learnt-Invariance VAE definition. Without Conjecture 3, i.e., with a simple P(Η|Z).
...
from src.models.livae import (
    make_livae_reconstruction_plot as make_livae_no_conj1_reconstruction_plot,
)
from src.models.livae import make_livae_sampling_plot as make_livae_no_conj1_sampling_plot
...
class LIVAE_NO_CONJ1(nn.Module):
    ...
    def __call__(self, x: Array, rng: PRNGKey, α: float = 1.0) -> Tuple[distrax.Distribution, ...]:
        ...
        p_Η_given_z = self.p_Η_given_Z_base(z)
...
def calculate_livae_no_conj1_elbo(
    x: Array,
...
def livae_no_conj1_loss_fn(
    ...
    loss, metrics = calculate_livae_no_conj1_elbo(
...
def make_livae_no_conj1_batch_loss(model, agg=jnp.mean):
    @jax.jit
