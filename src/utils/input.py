# Adapted from Uncertainty Baselines:
...
def _get_dataset_builder(
    dataset: Union[str, tfds.core.DatasetBuilder], data_dir: Optional[str] = None
    ...
    if isinstance(dataset, str):
        ...
        raise ValueError(
            f"`dataset` must be a string or tfds.core.DatasetBuilder. Received {dataset} instead."
        )
    return dataset_builder


def _get_process_split(
...
def _preprocess_with_per_example_rng(
    ds: tf.data.Dataset, preprocess_fn: Callable[[Features], Features], *, rng: jnp.ndarray
) -> tf.data.Dataset:
...
def _build_dataset(
    ...
    read_config = tfds.ReadConfig(shuffle_seed=file_shuffle_seed, options=dataset_options)
...
def get_data(
    ...
    if not drop_remainder:
        ...
        ds = ds.concatenate(padding_dataset.repeat(batch_size_per_device * num_devices - 1))
