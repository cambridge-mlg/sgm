...
def get_closest_value_in_sorted_sequence(
    value: T, seq: Sequence[T], error_on_out_of_bounds: bool = False
) -> T:
    """
    ...
    if pos == 0:
        if error_on_out_of_bounds and value < seq[0]:
            raise ValueError(
                f"Value {value} must be inside the list's range [{seq[0]}, {seq[-1]}]"
            )
        return seq[0]
    if pos == len(seq):
